...
//    const [paymentProduct, setPaymentProduct] = useState({});
   ...
   const {
     ...
     queryKey: ["allPlaceData"],
     queryFn: async () => {
       const res = await fetch(`http://localhost:5000/allPlaceData`);
   ...
   const deleteHandler = (id) => {
     ...
     fetch(`http://localhost:5000/allPlaceData/${id}`, {
       method: "DELETE",
     })
       .then((res) => res.json())
       .then((data) => {
                   ...
                   {allPlaceData?.map((allPlace) => (
                     <>
                       <tr key={allPlace?._id}>
                           ...
                           <div className="flex items-center space-x-3">
                             <div className="avatar">
                               <div className="mask mask-squircle w-12 h-12">
                                 <img
                                   src={allPlace?.picture}
                                   alt="Avatar Tailwind CSS Component"
                                 />
                               </div>
                             </div>
                             <div>
                               <div className="font-bold">{allPlace?.name}</div>
                         ...
                         <td>{allPlace?.category}</td>
                         <td className="font-bold">
                           ${allPlace?.perNightCost}
                         </td>
                         <td>
                           <button
