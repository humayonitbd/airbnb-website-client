...
const FavoritePlace = () => {
    ...
    const {
      ...
      queryFn: async () => {
        const res = await fetch(
          `http://localhost:5000/wishlistPlace/${user?.email}`
        );
    ...
    const deleteHandler = (id) => {
      ...
      fetch(`http://localhost:5000/wishlistPlace/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    const handleSubmitBtnModal = (e) => {
      ...
      const payInfo = {
        ...
        wishlistProductID: paymentProduct.wishlistProductID,
        payment: "paid",
      };

      fetch(
        `http://localhost:5000/wishlistPlacePayment/${paymentProduct._id}`,
        {
          method: "PUT",
          headers: {
    ...
    console.log(
      ...
      paymentProduct,
      "oldID",
      paymentProduct.wishlistProductID
    );
        ...
        <div className="pl-5 pt-5">
          <div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {myWishlistPlace?.length ? (
                  <>
                    {isLoading && <div> loading.... </div>}
                    {myWishlistPlace?.map((wishlistedPlace) => (
                      <>
                        <tr key={wishlistedPlace?._id}>
                          <td>
                            <div className="flex items-center space-x-3">
                              <div className="avatar">
                                <div className="mask mask-squircle w-12 h-12">
                                  <img
                                    src={wishlistedPlace?.picture}
                              ...
                              <div>
                                <div className="font-bold">
                                  {wishlistedPlace?.name}
                                </div>
                                <div className="text-sm opacity-50">
                                  {wishlistedPlace?.location}
                                </div>
                              </div>
                            </div>
                          </td>
                          <td>{wishlistedPlace?.category}</td>
                          <td className="font-bold">
                            ${wishlistedPlace?.perNightCost}
                          </td>
                          <td>
                            <button
                              onClick={() => deleteHandler(wishlistedPlace._id)}
                              className=" bg-red-500 text-white px-5 py-2 rounded mr-2"
                            >
                              Delete
                            </button>
                            {wishlistedPlace.payment ? (
                              <>
                              ...
                              <>
                                <button
                                  onClick={() => openPayModal(wishlistedPlace)}
